...
class NotesController < ApplicationController
  ...
  def procesar
        ...
        @boleta = Note.select("fecha,serie,numero,cod_cli,td,ruc,precio,precio_sigv,cod_prod,sum(cantidad) as cantidad, sum(importe) as importe").where(["fecha >= ? and fecha <= ? and importe >= 0 and procesado = ? and serie = ?", "#{fecha1} 00:00:00","#{fecha2} 23:59:59","0","BB04"] ).group(:fecha,:serie,:numero)
        ...
        BCP Cuenta Recaudadora Moneda Nacional : 191-2264838-0-49"  
                ...
                case_54 = ReceiptGenerator.new(8, 54, 1, "BB04").with_igv(true)      
                #  case_3 = InvoiceGenerator.new(1, 3, 1, "FA02").with_igv(true)
  ...
  # GET /notes.json
  def index
      @notes = Note.all.paginate(:page => params[:page]).order("fecha DESC")
  end
